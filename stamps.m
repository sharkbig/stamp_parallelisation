...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% start process data (the previous code is used to get progject property)  %%
...
if strcmpi(stamps_PART1_flag,'y')
  for i=1:length(patchdir)
    if ~isempty(patchdir(i).name)
      % get into the process directory
      cd(patchdir(i).name)
...
% step 6-8 cannot be parallelized.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmpi(stamps_PART2_flag,'y')
    ...
    % Step 7: stimate spatially-correlated look angle error
